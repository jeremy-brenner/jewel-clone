...
(function() {
  ...
  Menu = (function() {
    ...
    Menu.prototype.open = function(menu) {
      ...
      for (i = j = 0, len = ref.length; j < len; i = ++j) {
        item = ref[i];
        results.push(this.createItem(item, i, this.menu[menu].length));
      }
      return results;
    };

    Menu.prototype.createItem = function(item, i, t) {
      ...
      item.object.position.y = i * this.fontSize() * -2 + GEMGAME.realHeight() / 2 + t * this.fontSize() / 2;
    ...
    Menu.prototype.update = function(t) {
      ...
      if (GEMGAME.input.touching) {
        ...
        if (i !== false) {
          if (typeof (base = this.menu[this.current][i]).exec === "function") {
            base.exec();
          }
          return this.close();
        }
    ...
    Menu.prototype.checkTouch = function(ty) {
      ...
      if (ty > this.fontSize() + this.menu[this.current][0].object.position.y) {
        return false;
      }
      ref = this.menu[this.current];
      for (i = j = 0, len = ref.length; j < len; i = ++j) {
